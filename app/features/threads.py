...
@bot.tree.command(name="create-thread", description="Create a thread and move user messages to it.")
  ...
  ommitted_messages: str = ""
    ...
    if not is_mod(interaction.user):
        await interaction.response.send_message(
          "You do not have permission to use the create-thread command.", ephemeral=True
        )
        return
    ...
    if starting_message_id is not None and ending_message_id is not None:
        ...
        async for message in channel.history(limit=100, before=ending_message, after=starting_message):
        ...
        for message in message_list:
            ...
            if message.reference is not None:
                ...
                await webhook.send(content, username=message.author.name, avatar_url=message.author.avatar.url, thread=thread)
            # if message is not a reply just send it normally
            else:
                await webhook.send(message.content, username=message.author.name, avatar_url=message.author.avatar.url, thread=thread)
